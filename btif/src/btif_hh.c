    ...
    switch (event)
    {
        ...
        case BTA_HH_CLOSE_EVT:
            ...
            if (p_dev != NULL) {
                ...
                BTIF_TRACE_DEBUG("%s: local_vup = %d", __FUNCTION__, p_dev->local_vup);
                if (p_dev->local_vup)
                {
                   /* If it is locally initiated VUP, remove the bond as ACL
                    * got disconnected while VUP being processed.
                    */
